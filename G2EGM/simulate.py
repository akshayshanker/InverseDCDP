...
@njit
def euler2(sim,sol,par, w_eta_shocks):
    ...
    #w_eta_shocks = np.random.choice(len(par.w_eta), size=10000, replace=True, p=par.w_eta)

    for i in range(N):
        n = 0 
        eta_draw = w_eta_shocks[i,0]
        m = eta_draw
        # b. loop over time
        for t in range(par.T):
            ...
            euler[t,i] = np.log10(np.abs(euler_raw/c)+1e-16)

            m = par.Ra*a + w_eta_shocks[i,t+1]
            n = par.Rb*b
