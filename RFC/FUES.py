"""Fast Upper-Envelope Scan (FUES) copied from the author's reference
implementation. Provides a numba-jitted single pass envelope cleaner.
"""
...
def uniqueEG(egrid, vf):
    """
    Return a boolean mask that keeps the highest value for each rounded e-grid
    node. Used to drop duplicate endogenous grid points before refinement.
    """
...
def circ_put(buf, head, value):
    """Write value at head position in a circular buffer and advance head."""
    buf[head] = value
...
def seg_intersect(a1, a2, b1, b2):
    """Intersection of two line segments; returns (x, y) or (nan, nan)."""
...
def FUES(e_grid, vf, policy_1, policy_2, del_a,
    ...
    """Sort input, run the fast scan, drop NaNs, and return cleaned arrays."""
...
def _scan(e_grid, vf, a_prime, del_a,
    ...
    Implements:
    1. Pre-allocated scratch arrays for gradient scans.
    2. Circular buffer of recently dropped indices.
    """
